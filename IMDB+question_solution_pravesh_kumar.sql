...
use upgrad;
select * from movies_upgrad;
select * from director_mapping_upgrad;
select * from genere_upgrad;
select * from names_upgrad;
select * from role_mapping_upgrad;
select * from rating_upgrad;

desc names_upgrad;
show columns from movies_upgrad ;
...
SELECT 
		SUM(CASE WHEN id="" THEN 1 ELSE 0 END) AS ID_nulls, 
		SUM(CASE WHEN title ="" THEN 1 ELSE 0 END) AS title_nulls, 
		SUM(CASE WHEN year ="" THEN 1 ELSE 0 END) AS year_nulls,
		SUM(CASE WHEN date_published ="" THEN 1 ELSE 0 END) AS date_published_nulls,
		SUM(CASE WHEN duration ="" THEN 1 ELSE 0 END) AS duration_nulls,
		SUM(CASE WHEN country ="" THEN 1 ELSE 0 END) AS country_nulls,
		SUM(CASE WHEN worlwide_gross_income ="" THEN 1 ELSE 0 END) AS worlwide_gross_income_nulls,
		SUM(CASE WHEN languages ="" THEN 1 ELSE 0 END) AS languages_nulls,
		SUM(CASE WHEN production_company ="" THEN 1 ELSE 0 END) AS production_company_nulls
...
use upgrad;
select distinct year, count(year) as number_of_movies from movies_upgrad group by year order by year;

-- with pravesh_kumar as 
-- (select *, substr(date_published,4,2) as months from movies_upgrad)
-- select distinct months ,count(months) from pravesh_kumar group by months order by months;
...
select * from movies_upgrad;
select distinct country , count(country) from movies_upgrad   where year=2019 group by country having country in ("India","USA");
...
select  distinct genre from genere_upgrad;


-- select count(genre) from genere_upgrad group by genre ;
...
select distinct genre, count(genre) no_of_movies from genere_upgrad group by genre order by count(genre) desc limit 1;

-- or
-- SELECT g.genre, COUNT(g.genre) AS number_of_movies, m.year 
-- FROM movies_upgrad AS m
-- INNER JOIN
-- genere_upgrad AS g
-- ON m.id = g.movie_id
-- GROUP BY g.genre
...
-- Type your code below:
with pravesh_kumar as
(select count(genre) as no_of_movies from genere_upgrad  group by movie_id having count(genre)=1)
select count(no_of_movies) from pravesh_kumar;
...
-- select * from genere_upgrad;
-- select * from movies_upgrad;
select g.genre , avg(m.duration) avg_duration
   from genere_upgrad g
   inner join movies_upgrad m
   on g.movie_id=m.id
   group by genre;
...
/* Output format:
+---------------+-------------------+---------------------+----------------------+-----------------+-----------------+
| min_avg_rating|	max_avg_rating	|	min_total_votes   |	max_total_votes 	 |min_media n_rating|min_median_rating|
+---------------+-------------------+---------------------+----------------------+-----------------+-----------------+
|		0		|			5		|	       177		  |	   2000	    		 |		0	       |	8			 |
+---------------+-------------------+---------------------+----------------------+-----------------+-----------------+*/
-- Type your code below:
...
FROM names_upgrad AS n
INNER JOIN director_mapping_upgrad AS d
ON n.id=d.name_id
INNER JOIN movies_upgrad AS m
ON d.movie_id=m.id
INNER JOIN genere_upgrad AS g
ON m.id=g.movie_id
INNER JOIN rating_upgrad AS r
...
FROM role_mapping_upgrad rm
INNER JOIN names_upgrad n
ON n.id = rm.name_id
INNER JOIN rating_upgrad r
...
INNER JOIN rating_upgrad AS r 
ON m.id = r.movie_id 
INNER JOIN role_mapping_upgrad AS rm 
ON m.id=rm.movie_id 
INNER JOIN names_upgrad AS nm 
ON rm.name_id=nm.id
...
FROM movies_upgrad m
INNER JOIN genere_upgrad g
ON m.id = g.movie_id
INNER JOIN rating_upgrad r
ON r.movie_id = m.id
WHERE genre="thriller";
...
(
    ...
    INNER JOIN genere_upgrad AS g 
    ON m.id= g.movie_id
	WHERE genre IN (SELECT genre FROM pravesh_kumar)
)
...
FROM director_mapping_upgrad d
	 JOIN names_upgrad AS n 
     ON d.name_id=n.id 
	 JOIN movies_upgrad AS m 
     ON d.movie_id=m.id
),
...
(
 ...
 (
	 ...
	 FROM
		 names_upgrad AS n 
         JOIN director_mapping_upgrad AS d 
         ON n.id=d.name_id
		 JOIN rating_upgrad AS r 
