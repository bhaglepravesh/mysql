...
SELECT *,
		SUM(daily_total) OVER (ORDER BY ship_date ROWS UNBOUNDED PRECEDING) AS running_total,
...
update market_fact_full set profit =
( 	CASE profit
		WHEN profit <-500 THEN 'Huge Loss'
        WHEN profit BETWEEN -500 AND 0 THEN 'Bearable Loss'
        WHEN profit BETWEEN 0 AND 500 THEN 'Decent Profit'
...
select 
        ...
        count( order_number) as ord_number_count
 from 
      orders_dimen ;     
...
select
        ...
        p.Product_Sub_Category,
        sum(m.profit) as profits,
        round(sum(m.profit)/count(distinct order_number),2) as avg_profit_per_order
from market_fact_full m
      inner join prod_dimen p
     on p.prod_id=m.prod_id
...
select
        ...
        sum(m.profit) as profits,
        round(sum(m.profit)/count(distinct order_number),2) as avg_profit_per_order,
        round(sum(m.sales)/count(distinct order_number),2) as avg_sales_per_order,
