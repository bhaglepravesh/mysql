...
-- find the name  who will get maximum contries bonus , min_size<=family_size to get bonus

select * from countries where MIN_SIZE<=(select max(family_size) from families);

-- find the number of bonus to every person have
select distinct f.name,count(f.name) from families f
 inner join countries c on f.family_size>c.min_size group by f.name;


select distinct f.name,count(f.name) from families f
 inner join countries c on f.family_size between c.min_size and c.max_size group by f.name;