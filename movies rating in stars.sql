...
with cte as 
(select m.genre,m.title,avg(r.rating) avg_rating, repeat("*",round(avg(r.rating),0)) ratings,
row_number() over(partition by m.genre order by avg(r.rating) desc) as rn   from
movies m inner join reviews r on m.id=r.movie_id group by m.genre,m.title )
select genre,title,ratings from cte where rn=1 order by genre;